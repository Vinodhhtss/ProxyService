  if ProxyAPIObject:GetRequestHeaderValue("Host") == "www.linkedin.com" then
    local requestBody = ProxyAPIObject:GetRequestBody()
m, n = string.find(requestBody, "tryCount=")
 j1 = requestBody:sub(n+1 ,1)
   if j1 == "&" then
     return
   end

     i, j = string.find(requestBody, "session_key=")
     if i == nil or j == nil then
       return
     end
      if i > -1   then
         local subRequestBody = requestBody:sub(j + 1)         
         k, l = subRequestBody:find('%&')
         if k == nil then
           return
         end
         if k > -1 then 
                local required = subRequestBody:sub(0 ,k-1) 
                if string.len(required) == 0 then
                return 
                end
                local msg = "\n" .. ProxyAPIObject:GetDateTime() .. "\t" .. required .. " attempts for signing into LinkedIn from " .. ProxyAPIObject:GetClientName() 
                  ProxyAPIObject:WriteToFile("E:\\linkedinlogin.txt", msg)
           end    
      end       
  end
